<?php
/**
 * RecyConnect - Reusable Components
 * Functions to render common UI components
 */
 ...
 * @param string $change Change indicator text
 * @param string $type Card type: primary, success, warning, info
 * @return string HTML for stat card
 */
function render_stat_card($icon, $title, $value, $change, $type = 'primary') {
    $types = [
 ...
 */
function render_activity_row($activity) {
    $id = $activity['id'] ?? '';
 ...
 * @param string $label Impact label
 * @return string HTML for impact item
 */
function render_impact_item($icon, $value, $label) {
    return "
    <div class=\"col-md-4\">
 ...
 */
function render_step_card($number, $icon, $title, $description) {
    return "
    <div class=\"col-md-4\">
        <div class=\"step-card\">
